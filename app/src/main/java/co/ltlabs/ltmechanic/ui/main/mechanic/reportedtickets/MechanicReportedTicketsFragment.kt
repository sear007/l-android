...
import co.ltlabs.ltmechanic.databinding.FragmentMechanicReportedTicketsBinding
...
import co.ltlabs.ltmechanic.ui.adapter.MechanicReportedTicketsListAdapter
...
const val TAG = "MReportedFragment";

class MechanicReportedTicketsFragment : BaseFragment() {

    @Inject
    lateinit var languageJsonObject: JSONObject

    private val args: MechanicReportedTicketsFragmentArgs by navArgs()
    private lateinit var ticketType: TicketType
    private lateinit var binding: FragmentMechanicReportedTicketsBinding
    private val viewModel: MechanicReportedTicketsViewModel by viewModels { providerFactory }
    ...
    override fun onCreateView(
        ...
        binding = FragmentMechanicReportedTicketsBinding.inflate(inflater)
        binding.lifecycleOwner = viewLifecycleOwner

        binding.viewModel = viewModel
        binding.jTranslate = languageJsonObject

        val adapter = MechanicReportedTicketsListAdapter(viewModel, languageJsonObject)
        ...
        machineViewModel.status.observe(viewLifecycleOwner) { status ->
            when (status) {
                ApiStatus.LOADING -> {
        ...
        viewModel.mfgLinesFromDatabase.observe(viewLifecycleOwner, Observer { mfgLines ->
            ...
            ticketViewModel.getReportedTickets(
        ...
        ticketViewModel.commonProblems.observe(viewLifecycleOwner) {
            if (it != null) {
                dismissPopup()
                if (it.isNotEmpty()) {
        ...
        machineViewModel.status.observe(viewLifecycleOwner) {
            when (it) {
                ApiStatus.LOADING -> {
                    binding.progressBar.showProgressBar(true)
                }
        ...
        ticketViewModel.reportedTickets.observe(viewLifecycleOwner) { reportedTickets ->
            if (reportedTickets != null) {
                viewModel.getReportedOrReopenedTicket(ticketType, reportedTickets)
            }
        }

        viewModel.reportedTickets.observe(viewLifecycleOwner) { reportedTickets ->
        ...
        ticketViewModel.snackBarActionsFromDatabase.observe(viewLifecycleOwner) {
            ...
            if (it != null && it.isNotEmpty()) {
                ...
                when (it[0].action) {
                    ...
                    SNACK_BAR_ACTION_INSERT -> {
                        if (it[0].show) {
                            binding.coordinatorLayout.showSnackbar(
                                languageJsonObject.getTranslation("Station has been inserted.")
                            )
                    ...
                    SNACK_BAR_ACTION_REMOVE_MACHINE -> {
                        if (it[0].show) {
                            binding.coordinatorLayout.showSnackbar(
                                languageJsonObject.getTranslation("Machine has been removed.")
                            )
                    ...
                    SNACK_BAR_ACTION_KEEP_EMPTY_INSERT -> {
                        if (it[0].show) {
                            val message = "Empty station has been inserted"

                            binding.coordinatorLayout.showSnackbar(
                                languageJsonObject.getTranslation(message)
                            )
                        }
                    }
                    ...
                    SNACK_BAR_ACTION_CANCEL_TICKET -> {
                        if (it[0].show) {
                            val message = "Repair Ticket has been cancelled."
        ...
        viewModel.navigateToTicketPreview.observe(viewLifecycleOwner) {
            if (it != null) {
        ...
        ticketViewModel.status.observe(viewLifecycleOwner) {
            when (it) {
                ApiStatus.LOADING -> {
                    binding.progressBar.showProgressBar(true)
                }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setToolbarTitle()
        binding.btnCreateTicket.isVisible =
            UserType.convertToType(AuthUtil.role) !is UserType.Mechanic
    }
    ...
    private fun navigateToTicketPreview(
        ...
        val action = MechanicReportedTicketsFragmentDirections
            .actionMechanicReportedTicketsFragmentToMechanicReportedTicketsPreviewFragment(
                ticketId,
                ticketNo,
                machineNo,
                problem,
    ...
    private fun navigateToCreateTicket(
        ...
        val action =
            MechanicReportedTicketsFragmentDirections.actionMechanicReportedTicketsFragmentToMechanicCreateTicketFragment(
                machineId,
                machine,
                station,
                mfgLine,
    ...
    private fun navigateToReplace(
        ...
        val action = MechanicReportedTicketsFragmentDirections
            .actionMechanicReportedTicketsFragmentToReplaceMachineScanDetailsFragment(
                mfgLineId,
                mfgLine,
                station,
                machine,
    ...
    private fun navigateToMoveMachine(
        ...
        val action = MechanicReportedTicketsFragmentDirections
            .actionMechanicReportedTicketsFragmentToMoveMachineFragment(
                machineId,
                machine,
                rfid,
                subType,
    ...
    private fun navigateToQueryMachine(machineId: Long, machine: String) {
        val action = MechanicReportedTicketsFragmentDirections
            .actionMechanicReportedTicketsFragmentToQueryMachineFragment(machineId, machine)
        navigate(action)
    }
